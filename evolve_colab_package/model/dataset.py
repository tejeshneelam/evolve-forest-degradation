...
def interp_nan_1d(arr: np.ndarray) -> np.ndarray:
    """Linear interpolation of NaN values along a 1-D array (time axis)."""
    nans = np.isnan(arr)
    if nans.all():
...
def fill_nans(patch: np.ndarray) -> np.ndarray:
    """
    Fill NaNs in a (T, C, H, W) patch by interpolating each pixel's
    ...
    for c in range(C):
        for h in range(H):
            for w in range(W):
                out[:, c, h, w] = interp_nan_1d(out[:, c, h, w])
    return out
